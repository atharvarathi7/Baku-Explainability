...
def load_pickle_file():
    ...
    try:
        ...
        create_resizable_content(frame4, None, plot_func3, data["features_gradient_weighted_tokens"], 0, 1,"Grad Weighted Action Tokens w.r.t Action Features")
        create_resizable_content(frame4, None, plot_func3, data["action_gradient_weighted_tokens"], 0, 0, "Grad Weighted Action Tokens w.r.t Action")
...
def update_display(value):
    global data
    i = int(value)
    create_resizable_content(frame1, None, plot_func4, data["generic_atten_wrt_action_gradient"][i], 0, 0,"Generic Attention Map wrt Action")
    create_resizable_content(frame1, None, plot_func4, data["generic_atten_wrt_action_features_gradient"][i], 0, 1,"Generic Attention Map wrt Action Features")
...
def open_plot_window(key1):
    ...
    if key1 == 'last_layer_atten_maps':
        title = "Attention Map without Grad"
        plot_window.title(title)

        fig, ax = plt.subplots()
        cax = ax.matshow(data1[0], cmap='viridis')
        fig.colorbar(cax)
        row_labels = ['Text', 'Pix', 'Ego Pix', 'Proprio', 'Action']
        col_labels = ['Text', 'Pix', 'Ego Pix', 'Proprio', 'Action']
        ...
        ax.set_xlabel('Timesteps')
        ax.set_ylabel('Token Values')
        ax.set_title('Last layer attention')

        fig.tight_layout()
        fig.canvas.draw()

    else:
        title = "Attention Map Tokens without Grad"
        plot_window.title(title)
...
action_button = ttk.Button(main_frame, text="Attn Map", command= lambda: open_plot_window('last_layer_atten_maps'))
action_button.grid(row=3, column=3, padx=5)

feat_button = ttk.Button(main_frame, text="Action Tokens", command=lambda: open_plot_window('atten_tokens'))
