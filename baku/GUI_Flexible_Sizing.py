        ...
        try:
            ...
            for item in self.data:
                ...
                for key, val in item.items():
                    if key == "Agent View":  # Assume key 'Agent View' contains pixel data
                        image_label = self.display_image(frame, val)
                        image_label.pack()
                        image_text_label = tk.Label(frame, text="Agent View")
                        image_text_label.pack()
                    elif key == "attn_maps":  # Assume key 'attention_data' contains 5x5 self-attention data
                        heatmap_label = self.display_heatmap(frame, val[0][0][0])
                        heatmap_label.pack()
                        heatmap_text_label = tk.Label(frame, text="Attention Heatmap")
                        heatmap_text_label.pack()
                    else:
