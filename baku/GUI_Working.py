...
class PickleViewerApp:
    ...
    def load_pickle_file(self):
        ...
        try:
            ...
            for item in self.data:
                ...
                # Display image and heatmap in the same row
                #if "Agent View" in item and "attn_maps" in item:
                image_label = self.display_image(frame, item["Agent View"])
                image_label.grid(row=0, column=0, padx=10, pady=10)

                heatmap_label = self.display_heatmap(frame, item["attn_maps"][0][0][0])
                heatmap_label.grid(row=0, column=1, padx=10, pady=10)
                
                # Labels for image and heatmap
                ...
                #     text_widget.insert(tk.END, f"No image or heatmap data found.\n")
    ...
    def display_heatmap(self, parent, attention_data):
        ...
        resized_heatmap_image = heatmap_image.resize((256, 256), Image.Resampling.LANCZOS)

        photo = ImageTk.PhotoImage(resized_heatmap_image)
        buf.close()
    ...
    def update_display(self, value):
        ...
        if 0 <= index < len(self.frames):
            ...
            self.frames[index].pack(fill=tk.BOTH, expand=True)  # Show the current frame
        else:
